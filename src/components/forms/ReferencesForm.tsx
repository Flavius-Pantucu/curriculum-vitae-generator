...
export const ReferencesForm = ({ references, setReferences }: ReferencesProps) => {
    ...
    const handleAdd = () => {
        if (references.length < 2) {
            setReferences([...references, { name: "", title: "", company: "", email: "", phone: "" }]);
        }
    };
    ...
    const fields: (keyof Reference)[] = ["name", "title", "company", "email", "phone"];
    ...
    return (
        <div>
            ...
            <Button type="primary" disabled={references.length >= 2} onClick={handleAdd} block>
                Add Reference
            </Button>
        </div>
    );
